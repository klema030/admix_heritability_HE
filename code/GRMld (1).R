...
# Binary file for .grm.bin
con = file(paste0(grmdir, "/", filename,".grm.bin"), "wb")
writeBin(A_grm, con, size=4)
close(con)

# Binary file for .grm.N.bin, number of loci
con_N = file(paste0(grmdir, "/", filename, ".grm.N.bin"), "wb")
writeBin(m,con_N,size=4)
close(con_N)
